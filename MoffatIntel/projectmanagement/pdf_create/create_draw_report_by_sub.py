...
def create_draw_report_by_sub(project, draws, sub, vendor, checks, invoices, exhibits, line_items, groups):
    ...
    for group in groups:
        ...
        for line_item in line_items:
            if line_item.group_id == group.id:
                bldg_contract += line_item.total

        total_contract += bldg_contract
        ...
        try:
            bldg_pct_paid = paid_total/bldg_contract
        except:
            pass
        ...
        pdf.set_xy(10, pdf.get_y())
        pdf.cell((pdf.w - 20) * .14, 5, '', 0, 0, 'C', True)
        pdf.cell((pdf.w - 20) * .14, 5, f'${bldg_contract:.2f}', 0, 0, 'C', True)
        pdf.cell((pdf.w - 20) * .14, 5, f'${paid_total:.2f}', 0, 0, 'C', True)
        pdf.cell((pdf.w - 20) * .14, 5, '', 0, 0, 'C', True)
        pdf.cell((pdf.w - 20) * .14, 5, f'${bldg_retention:.2f}', 0, 0, 'C', True)
        pdf.cell((pdf.w - 20) * .14, 5, f'{bldg_pct_paid:.2f}%', 0, 0, 'C', True)
        pdf.cell((pdf.w - 20) * .14, 5, f'${bldg_payable:.2f}', 0, 0, 'C', True)
    ...
    try:
        total_pct_paid = total_paid/total_contract
    except:
        pass
    ...
    pdf.cell((pdf.w - 20) * .14, 5, f'${total_contract:.2f}', 0, 0, 'C', True)
    pdf.cell((pdf.w - 20) * .14, 5, f'${total_paid:.2f}', 0, 0, 'C', True)
    pdf.cell((pdf.w - 20) * .14, 5, '', 0, 0, 'C', True)
    pdf.cell((pdf.w - 20) * .14, 5, f'${total_retention:.2f}', 0, 0, 'C', True)
    pdf.cell((pdf.w - 20) * .14, 5, f'{total_pct_paid:.2f}%', 0, 0, 'C', True)
    pdf.cell((pdf.w - 20) * .14, 5, f'${total_payable:.2f}', 0, 0, 'C', True)
